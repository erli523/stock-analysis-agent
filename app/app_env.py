...
def use_current_python_environment():
    """Use the active conda/venv instead of creating the project .venv."""
    explicit = os.environ.get("HENGLINE_USE_CURRENT_ENV", "").lower()
...
WINDOWS_REQUIREMENTS_FILE = os.path.join(PROJECT_ROOT, "requirements-windows.txt")
if os.path.exists(WINDOWS_REQUIREMENTS_FILE):
    REQUIREMENTS_FILE = WINDOWS_REQUIREMENTS_FILE
...
class AppBaseEnv:
    ...
    def check_python_installation(self):
        """步骤1: 检查Python是否安装"""
        info("=== 检查Python环境 ===")
        python_command = f'"{sys.executable}"' if use_current_python_environment() else "python"
        result = self.run_command(f"{python_command} --version", capture_output=True)
    ...
    def create_virtual_environment(self):
        """步骤2: 检查并创建虚拟环境"""
        info("=== 检查虚拟环境 ===")
        if use_current_python_environment():
            info(f"Using active Python environment instead of {VENV_DIR}: {sys.executable}")
            return True
    ...
    def activate_virtual_environment():
        ...
        debug("=== 步骤3: 获取虚拟环境路径 ===")
        if use_current_python_environment():
            debug(f"[success] Using active Python environment: {sys.executable}")
    ...
    def install_dependencies(self, pip_exe):
        ...
        # 使用虚拟环境的pip安装项目依赖
        if pip_exe:
            install_command = f'"{pip_exe}" install -r "{REQUIREMENTS_FILE}"'
        else:
