...
class DocumentRetriever:
    ...
    def _init_retriever(self, **kwargs):
        ...
        try:
            ...
            self.retriever = None
            self.query_engine = None
            # 添加更安全的错误处理，不再抛出异常，避免影响上层调用
            # raise  # 注释掉raise以确保在初始化失败时不会中断程序流程
    ...
    def retrieve(self, query: str, **kwargs) -> List[NodeWithScore]:
        ...
        try:
            ...
            # 处理top_k参数
            if 'top_k' in kwargs:
                # 临时保存原始的similarity_top_k
                original_top_k = self.similarity_top_k
                # 更新为传入的top_k
                self.similarity_top_k = kwargs['top_k']
                # 重新获取检索器
        ...
        try:
            ...
            for node in nodes:
                try:
                    ...
                    debug(f"处理节点时出错: {str(e)}")
                    
            debug(f"成功检索并处理{len(results)}个文档")
            return results
        except Exception as e:
    ...
    def batch_retrieve(self, queries: List[str], **kwargs) -> List[List[Dict[str, Any]]]:
        ...
        for query in queries:
            try:
                result = self.retrieve_as_dict(query, **kwargs)
                results.append(result)
            except Exception as e:
                error(f"处理查询 '{query[:30]}...' 时出错: {str(e)}")
                results.append([])
        
        debug(f"成功完成{len(queries)}个查询的批量检索")
        return results
