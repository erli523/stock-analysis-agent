...
class VectorStoreManager:
    ...
    def __init__(self, vector_store_path=None, embedding_model=None):
        ...
        try:
            ...
            debug(f"尝试加载嵌入模型: {self.embedding_model}")
            ...
            debug(f"成功加载嵌入模型: {self.embedding_model}")
            ...
            try:
                ...
                try:
                    ...
                    debug("成功创建GPT4AllEmbeddings作为替代")
                except Exception as gpt4all_err:
                    warning(f"GPT4AllEmbeddings创建失败: {str(gpt4all_err)}")
                    ...
                    try:
                        ...
                        debug("成功创建OllamaEmbeddings作为替代")
                    except Exception as ollama_err:
                        warning(f"OllamaEmbeddings创建失败: {str(ollama_err)}")

                        # 如果所有替代方案都失败，创建一个更强大的模拟嵌入类
                        warning("无法创建替代嵌入模型。创建增强的模拟嵌入模型...")
                        ...
                        class EnhancedMockEmbeddings(Embeddings):
                            ...
                            def __init__(self, embedding_dim=384):
                                ...
                                debug(f"创建增强模拟嵌入模型 (维度: {embedding_dim})")
                        ...
                        debug("已创建增强的模拟嵌入模型，用于在离线环境下提供基本功能")
            except Exception as e:
                error(f"创建替代嵌入模型时出现意外错误: {str(e)}")
    ...
    def load_documents(self, data_dir='data'):
        ...
        debug(f"加载了 {len(documents)} 个文档")

        # 确保至少加载了一个文档
        if not documents:
            warning(f"在 {data_dir} 目录下未找到任何txt文件")
    ...
    def split_documents(self, documents):
        ...
        debug(f"正在将 {len(documents)} 个文档分割成小块...")
        splits = text_splitter.split_documents(documents)
        debug(f"分割完成，共生成 {len(splits)} 个文档块")
        return splits
    ...
    def create_vector_store(self, splits):
        ...
        debug(f"向量存储已保存到 {self.vector_store_path}")
    ...
    def load_vector_store(self):
        ...
        if os.path.exists(index_faiss_path) and os.path.exists(index_pkl_path):
            try:
                ...
                debug(f"向量存储已从 {self.vector_store_path} 加载")
            except Exception as e:
                error(f"加载向量存储时出错: {str(e)}")
                debug("将重新创建向量存储...")
                # 尝试重新创建向量存储
            ...
            warning(f"向量存储文件不存在: {self.vector_store_path}")
            # 确保向量存储目录存在
            if not os.path.exists(self.vector_store_path):
                os.makedirs(self.vector_store_path)
                debug(f"已创建向量存储目录: {self.vector_store_path}")
            ...
            if documents:
                ...
                warning("没有找到文档来创建向量存储。请确保data目录下有文本文件。")
                # 创建一个空的向量存储作为占位符
    ...
    def _create_empty_vector_store(self):
        ...
        try:
            ...
            if hasattr(self.embeddings, 'embed_documents'):
                ...
                self.vector_store.save_local(self.vector_store_path)
                debug(f"已创建空向量存储作为占位符: {self.vector_store_path}")
        except Exception as e:
            error(f"创建空向量存储时出错: {str(e)}")
            debug("将创建一个简单的模拟向量存储类")
    ...
    def retrieve(self, query, k=4):
        ...
        if self.vector_store:
            ...
            debug(f"正在进行相似度搜索，查询: {query_text}...")
            ...
            debug(f"检索完成，找到 {len(relevant_docs)} 个相关文档")
            return relevant_docs
        else:
            warning("向量存储未初始化，无法进行检索")
            return []
    ...
    def update_vector_store(self, data_dir='data'):
        ...
        Returns:
            更新后的向量存储
        """
        debug(f"开始更新向量存储，数据目录: {data_dir}")
...
if __name__ == "__main__":
    ...
    for i, doc in enumerate(relevant_docs):
        debug(f"\n文档 {i + 1}: ")
        debug(f"内容: {doc.page_content[:200]}...")
        if 'source' in doc.metadata:
            debug(f"来源: {doc.metadata['source']}")
        else:
            debug("来源: 未知")
